...
// Enums for quests requiring areatrigger scripts in elwynn
enum ElwynnForest
{
...
// The Fargodeep Mine Areatrigger for Quest ID: 62
class at_fargodeep_mine : public AreaTriggerScript
{
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/)
    {
        if (player->GetQuestStatus(QUEST_FARGODEEP_MINE) == QUEST_STATUS_COMPLETE)
        {
            return 0;
        }

        else if (player->HasQuest(QUEST_FARGODEEP_MINE))
        {
            player->CompleteQuest(QUEST_FARGODEEP_MINE, false);
        }
        return true;
    }
};

// The Jasperlode Mine Areatrigger for Quest ID: 76
class at_jasperlode_mine : public AreaTriggerScript
{
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/)
    {
        if (player->GetQuestStatus(QUEST_JASPERLODE_MINE) == QUEST_STATUS_COMPLETE)
        {
            return 0;
        }

        else if (player->HasQuest(QUEST_JASPERLODE_MINE))
        {
            player->CompleteQuest(QUEST_JASPERLODE_MINE, false);
        }
        return true;
    }
};
