...
public:
	...
	void OnUpdateZone(Player* p_Player, uint32 p_NewZoneID, uint32 p_OldZoneID, uint32 p_NewAreaID) override
	{
		...
		// Finding a Foothold
		if (p_NewAreaID == 7078 && p_Player->HasQuest(34582))
			p_Player->SetPhaseMask(4, true);

		// For The Alliance
		if (p_Player->HasQuest(34583) && p_Player->GetAreaId() == 7078)
		{
			p_Player->SetPhaseMask(8, true);
		}
	}
	
	void OnUpdateArea(Player* p_Player, uint32 p_NewZoneID, uint32 p_OldZoneID, uint32 p_NewAreaID, uint32 p_oldAreaID)
	{
		// Finding a Foothold
		if (p_oldAreaID == 7760 && p_NewAreaID == 7078 && p_Player->HasQuest(34582))
		{
			p_Player->SetPhaseMask(4, true);
		}

		// For The Alliance
		if (p_NewAreaID == 7078 && p_Player->HasQuest(34583))
		{
			p_Player->SetPhaseMask(8, true);
		}
	}
	...
	void OnLogin(Player* p_Player) override
	{
		...
		// If quest Finding a Foothold is in quest log or has been rewarded set phase to 4 
		if (p_Player->HasQuest(34582) || p_Player->GetQuestStatus(34582) == QUEST_STATUS_REWARDED)
		{
			p_Player->SetPhaseMask(4, true);
		}

		// For the alliance
		if (p_Player->HasQuest(34583) || p_Player->GetQuestStatus(34583) == QUEST_STATUS_REWARDED)
		{
			p_Player->SetPhaseMask(8, true);
		}
	}
	...
	{
		if (p_Player->GetQuestStatus(34582) == QUEST_STATUS_COMPLETE && p_Player->GetAreaId() == 7078)
		{
			p_Player->SetPhaseMask(4, true);
		}

		if (p_Player->GetQuestStatus(34583) == QUEST_STATUS_COMPLETE && p_Player->GetAreaId() == 7078)
		{
			p_Player->SetPhaseMask(8, true);
		}
	}
